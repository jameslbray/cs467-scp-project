...
async def get_user_from_api(user_id: str, token: str) -> UserInfo:
    ...
    users_service_url = (
        "http://localhost:8001"  # Can be moved to config settings
    )
...
def get_connection_manager():
    ...
    if connection_manager is None:
        raise HTTPException(
            status_code=503, detail="Connection service not initialized"
        )
    return connection_manager


@router.get("/connect/health", response_model=Dict[str, str])
async def health_check(
    ...
    return (
        {"status": "healthy"}
        if await connection_manager.check_connection_health()
        else {"status": "unhealthy"}
    )


@router.get("/connect/all", response_model=list[Connection])
...
async def create_connection(
    ...
    logger.info(
        f"Creating connection from {connection_data.user_id} to {connection_data.friend_id}"
    )
    ...
    if getattr(connection_data, "status", None) == "pending":
        try:
            ...
            logger.info(f"Created connection: {connection}, {connection_data}")
...
async def update_connection(
    ...
    """
    # Allow update if current user is either the sender or recipient
    if str(current_user) not in [
        str(connection_data.user_id),
        str(connection_data.friend_id),
    ]:
        raise HTTPException(
            status_code=HTTP_403_FORBIDDEN,
            detail="You can only update connections you are a part of",
        )

    logger.info(
        f"Updating connection {connection_data.user_id} -> {connection_data.status} -> {connection_data.friend_id}"
    )

    try:
        # Call the manager to update connection status
        original_connection = await connection_manager.get_connection(
            connection_data.user_id, connection_data.friend_id
        )
        ...
        if (
            original_connection
            and original_connection.status == "pending"
            and connection_data.status == "accepted"
        ):
            try:
                # Get user information
                accepter_info = await get_user_from_api(
                    connection_data.user_id, token
                )
                ...
                logger.info(
                    f"Published friend acceptance notification to {recipient_id}"
                )
            except Exception as e:
