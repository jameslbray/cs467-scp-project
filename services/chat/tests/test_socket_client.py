    ...
    try:
        ...
        for i in range(max_retries):
            ...
            if i == max_retries - 1:
                ...
                raise RuntimeError(
                    f"Server health check failed after {max_retries} attempts"
                )
...
class TestSocketIOEvents:
    ...
    async def test_join_room(self, server_process, socket_client):
        ...
        assert (
            socket_client.room_joined or True
        ), "Failed to join room or no confirmation received"
    ...
    async def test_send_message(self, server_process, socket_client):
        ...
        assert any(
            msg.get("text") == test_message["text"]
            for msg in socket_client.received_messages
    ...
    async def test_leave_room(self, server_process, socket_client):
        ...
        assert (
            len(socket_client.received_messages) == 0
        ), "Should not receive messages after leaving the room"
    ...
    async def test_presence_request(self, server_process, socket_client):
        ...
        await socket_client.client.emit(
            "presence_request_friend_statuses", presence_request
        )
        ...
        if socket_client.presence_data is not None:
            assert isinstance(
                socket_client.presence_data, dict
            ), "Presence response should be a dictionary"
        else:
            logger.warning(
                "No presence response received. This may be expected if the feature is not implemented."
            )
    ...
    async def test_full_flow(self, server_process, socket_client):
        ...
        await socket_client.client.emit(
            "presence_request_friend_statuses", {"user_id": TEST_USER}
        )
        await asyncio.sleep(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-server",
        action="store_true",
        help="Don't start the server, only run tests",
    )
                    ...
                    try:
                        ...
                        if retry == 9:  # Last retry
                            ...
                            if server_proc.poll() is not None:
                                ...
                                logger.error(
                                    f"Server failed to start! Exit code: {server_proc.returncode}"
                                )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"Connection error: {e}")
                    logger.warning(
                        "Is the Socket.IO server running? Use without --no-server to start it automatically."
                    )
                except Exception as e:
