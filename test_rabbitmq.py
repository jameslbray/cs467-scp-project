...
async def consume_message(queue_name, connection_url, exchange_name=None, routing_key=None):
    """
    ...
    if exchange_name and routing_key:
        ...
        logger.info(f"Ensured binding between queue '{queue_name}' and exchange '{exchange_name}' with key '{routing_key}'")
...
async def bind_queue_to_exchange(connection_url, queue_name, exchange_name, routing_key):
    ...
    try:
        ...
        logger.info(f"Successfully bound queue '{queue_name}' to exchange '{exchange_name}' with routing key '{routing_key}'")
...
async def test_rabbitmq():
    ...
    try:
        ...
        logger.info(f"Binding queue '{client.user_events_queue}' to exchange '{client.exchange_name}'...")
        ...
        if message_body:
            try:
                ...
                assert message_data["event_type"] == test_event_type, "Event type mismatch"
                assert message_data["user_data"]["id"] == test_user_data["id"], "User ID mismatch"
                logger.info("✅ Test successful! Message published and consumed correctly.")
